...
def main():
    ...
    while True:
        if current_guess is None:
            current_guess = solver.suggest_guess()
        if current_guess is None:
            print("No se encontraron palabras posibles con la información dada.")
            break
        print(f"Intento {round_num}: Sugiero la palabra '{current_guess.upper()}'")
        feedback = input("Introduce el feedback para cada letra (v=verde, a=amarillo, n=negro), 'c' para cambiar palabra sugerida, o 'e' para escribir tu propia palabra: ").strip().lower()
        if feedback == "c":
            # Remove current guess from possible words and suggest next
            if current_guess in solver.possible_words:
                solver.possible_words.remove(current_guess)
            current_guess = None
            continue
        if feedback == "e":
            user_word = input("Escribe la palabra que quieres usar: ").strip().lower()
            if len(user_word) == 5 and user_word in solver.possible_words:
                current_guess = user_word
            else:
                print("Palabra inválida o no está en la lista de palabras posibles.")
                continue
        elif feedback == "vvvvv":
            print("¡Felicidades! La palabra ha sido adivinada.")
            break
        elif len(feedback) != 5 or not all(c in 'van' for c in feedback):
            print("Feedback inválido. Por favor, introduce una cadena de 5 caracteres usando solo v, a, n.")
            continue
        else:
            solver.update_feedback(current_guess, feedback)
            current_guess = None
            round_num += 1
